...
namespace NetDemo
{
    class SQLHelper
    {
        ...
        {
            string datebase = _db;
            string connectionString = ConfigurationManager.AppSettings[_db].ToString();
            //string connectionString1 = ConfigurationManager.AppSettings["constring1"].ToString();
            //string connectionString=  ConfigurationManager.ConnectionStrings[datebase].ConnectionString;
        ...
        public int Execute(string sql, SqlParameter[] parameter)
        {
            ...
            {
                loginfo.Error("Execute", ex);
                loginfo.Info("Execute:"+ sql);
                return -1;
            }
            finally
            {   ///关闭连接
